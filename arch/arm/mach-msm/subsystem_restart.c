...
int subsystem_restart_only_dev(struct subsys_device *dev)
{
	const char *name;
...
EXPORT_SYMBOL(subsystem_restart_only_dev);

int subsystem_restart_only(const char *name)
{
	int ret;
	struct subsys_device *dev = find_subsys(name);
	...
	ret = subsystem_restart_only_dev(dev);
	put_device(&dev->dev);
	return ret;
}
EXPORT_SYMBOL(subsystem_restart_only);
