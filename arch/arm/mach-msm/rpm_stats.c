...
/* keep a copy of the platform data that will be used later */
static struct msm_rpmstats_platform_data *saved_platform_data;
static void __iomem *rpm_reg = NULL;
...
#ifdef CONFIG_AMAZON_POWER_DEBUG
/*
   return the RPM mode counts
   Note that we hardcoded to read the vmin mode only
*/
int msm_rpmstats_read(u32 *vmin_count)
{
        if (!saved_platform_data){
                pr_err("%s: ERROR saved_platform_data is invalid.\n", __func__);
                return -1;
        }

	if (rpm_reg == NULL){
		rpm_reg = ioremap_nocache(saved_platform_data->phys_addr_base,
					  saved_platform_data->phys_size);

		if (!rpm_reg){
			pr_err("%s: ERROR could not ioremap  start=%p, len=%u\n",
			       __func__, (void*)saved_platform_data->phys_addr_base,
			       saved_platform_data->phys_size);
			return -1;
		}
	}

        *vmin_count = msm_rpmstats_read_long_register_v2(rpm_reg, 1,
                                                    offsetof(struct msm_rpm_stats_data_v2, count));

#if 0
        *last_entered = msm_rpmstats_read_quad_register_v2(rpm_reg,
                                                           1, offsetof(struct msm_rpm_stats_data_v2,
                                                                       last_entered_at));
        *last_exited = msm_rpmstats_read_quad_register_v2(rpm_reg,
                                                          1, offsetof(struct msm_rpm_stats_data_v2,
                                                                      last_exited_at));

        *accumulated = msm_rpmstats_read_quad_register_v2(rpm_reg,
                                                          1, offsetof(struct msm_rpm_stats_data_v2,
                                                                      accumulated));
        /* free the memory */
        iounmap(rpm_reg);
...
EXPORT_SYMBOL(msm_rpmstats_read);
#endif  
...
#ifdef CONFIG_AMAZON_POWER_DEBUG
        /* if everything goes fine, then save the platform data localy */
	saved_platform_data = pdata;
#endif
