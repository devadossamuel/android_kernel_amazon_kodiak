...
#if !defined(CONFIG_ARCH_MSM8974_APOLLO) && !defined(CONFIG_MACH_URSA)
	CLK_LOOKUP("cam_src_clk", mclk0_clk_src.c, "6c.qcom,camera"),
#endif
	CLK_LOOKUP("cam_src_clk", mclk1_clk_src.c, "90.qcom,camera"),
#if !defined(CONFIG_ARCH_MSM8974_APOLLO) && !defined(CONFIG_ARCH_MSM8974_THOR) && !defined(CONFIG_MACH_URSA)
...
#endif
#if defined(CONFIG_MACH_URSA)
	struct clk_lookup *imx_ois_src_clk = NULL, *imx_ois_clk = NULL;

	rfc_src_clk = clkdev_alloc(&mmss_gp0_clk_src.c, "cam_src_clk", "20.qcom,camera");
	imx_ois_src_clk = clkdev_alloc(&gp1_clk_src.c, "cam_src_clk2", "20.qcom,camera");
	rfc_clk = clkdev_alloc(&camss_gp0_clk.c, "cam_clk", "20.qcom,camera");
	ffc_src_clk = clkdev_alloc(&mmss_gp0_clk_src.c, "cam_src_clk", "6c.qcom,camera");
	ffc_clk = clkdev_alloc(&camss_gp0_clk.c, "cam_clk", "6c.qcom,camera");
	imx_ois_clk = clkdev_alloc(&gcc_gp1_clk.c, "cam_clk2", "20.qcom,camera");

	if (!rfc_src_clk || !rfc_clk || !ffc_src_clk || !ffc_clk || !imx_ois_src_clk || !imx_ois_clk) {
		printk(KERN_ERR"%s: failed to alloc camera clocks\n", __func__ );
	} else {
		clkdev_add(rfc_src_clk);
		clkdev_add(rfc_clk);
		clkdev_add(ffc_src_clk);
		clkdev_add(ffc_clk);
		clkdev_add(imx_ois_src_clk);
		clkdev_add(imx_ois_clk);
	}
#endif
/* front camera */
