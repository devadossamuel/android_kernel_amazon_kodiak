...
#define MAX_POOL_PAGES (36864) /* max pool size 144MB */
...
static unsigned int ion_iommu_heap_free(struct ion_buffer *buffer)
{
	...
	for (i = 0; i < num_orders; i++) {
		struct ion_page_pool *pool = iommu_heap->pool[i];
		nr_total += ion_page_pool_shrink(pool, __GFP_HIGHMEM, 0);
	}

	if (nr_total >= MAX_POOL_PAGES)
...
static int ion_iommu_heap_shrink(struct shrinker *shrinker,
...
struct ion_heap *ion_iommu_heap_create(struct ion_platform_heap *heap_data)
{
	...
	for (i = 0; i < num_orders; i++) {
		...
		iommu_heap->pool[i] = pool;
	}

	iommu_heap->heap.shrinker.shrink = ion_iommu_heap_shrink;
	iommu_heap->heap.shrinker.seeks = DEFAULT_SEEKS;
	iommu_heap->heap.shrinker.batch = 0;
	register_shrinker(&iommu_heap->heap.shrinker);
...
err_create_pool:
	for (i = 0; i < num_orders; i++)
		if (iommu_heap->pool[i])
			ion_page_pool_destroy(iommu_heap->pool[i]);

	kfree(iommu_heap->pool);
err_alloc_pool:
	kfree(iommu_heap);
	return ERR_PTR(-ENOMEM);
}
