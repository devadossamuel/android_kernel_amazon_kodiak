...
static int __devinit msm_serial_hsl_probe(struct platform_device *pdev)
{
	...
	/* Need to configure gpio's prior to adding the uart port, which
	 * will then print the console logs up to this point.  If we can't
	 * get the gpio pins, we return an error.
	 */
	if (pdata && pdata->config_gpio) {
		ret = msm_hsl_config_uart_gpios(port);
		if (ret) {
			pr_err("Failed to configure gpios for uart.\n");
			msm_hsl_unconfig_uart_gpios(port);
			return ret;
		}
	}
...
#ifdef CONFIG_MACH_URSA
	/* Turn off gpios if console not enabled
	 * or if uart_add_one_port failed
	 */
	if (ret || !port->cons || !(port->cons->flags & CON_ENABLED)) {
		if (pdata && pdata->config_gpio)
			msm_hsl_unconfig_uart_gpios(port);
	}
#endif
