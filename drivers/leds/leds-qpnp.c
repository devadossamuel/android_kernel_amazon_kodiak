...
#define WLED_MODULATION_SCHEME(base)		(base + 0x4A)
...
#define DOWNSCALE_MAX_CURRENT		1
#endif

// At <= this threshold, the FULL_SCALE current will be set to a max of
// WLED_DOWNSCALED_CURRENT
#define WLED_DOWNSCALE_THRESHOLD_INPUT	210
#define WLED_DOWNSCALE_MIN_INPUT	16
#define WLED_DOWNSCALED_CURRENT		3
//75 out of 255, scaled up to a 4095 scale
#define WLED_MAX_DUTY_CYCLE_DOWNSCALED	1204
//11 out of 255, scaled up to a 4095 scale
#define WLED_MIN_DUTY_CYCLE_DOWNSCALED	176
...
#ifdef CONFIG_MACH_URSA
//Used to ensure that the backlight doesn't turn on before the display has
// been on for 50+ ms, to avoid seeing a flash of garbage on the screen
DECLARE_WAIT_QUEUE_HEAD(ursa_display_on_waitqueue);
int ursa_waitqueue_shutdown = 0; //Nonzero indicates we should shut down now and not wait again
int ursa_display_enabled = 1;
#endif

static int qpnp_wled_set(struct qpnp_led_data *led)
{
#ifdef DOWNSCALE_MAX_CURRENT
	static int max_curr_downscaled = 0;
#endif
#ifdef CONFIG_MACH_URSA
	static int level_previously_off = 0;
#endif
	int rc, duty, level;
	...
	if (level == 0) {
...
#ifdef CONFIG_MACH_URSA
		level_previously_off = 1;
#endif
	} else {

#ifdef DOWNSCALE_MAX_CURRENT
		//Scale down the LED sink current from a 20mA max to
		// 5mA to allow for a higher PWM duty cycle and reduce
		// flickering at lower brightness levels
		if (level <= WLED_DOWNSCALE_THRESHOLD_INPUT)
		{
			if (!max_curr_downscaled)
			{
				//set max to WLED_DOWNSCALED_CURRENT
				rc = qpnp_led_masked_write(led,
					WLED_FULL_SCALE_REG(led->base, 0), WLED_MAX_CURR_MASK,
					WLED_DOWNSCALED_CURRENT);
				if (rc) {
					dev_err(&led->spmi_dev->dev,
		...
		else
		{
			if (max_curr_downscaled)
			{
				//set max back to the normal value
				rc = qpnp_led_masked_write(led,
					WLED_FULL_SCALE_REG(led->base, 0), WLED_MAX_CURR_MASK,
					led->max_current);
				if (rc) {
				...
				max_curr_downscaled = 0;
			}
		}
#endif //DOWNSCALE_MAX_CURRENT

#ifdef CONFIG_MACH_URSA
		//Wait until the display is turned on +50ms to avoid turning the backlight on
		// too early and showing garbage on the screen
		if (level_previously_off)
		{
			level_previously_off = 0;
			dev_dbg(&led->spmi_dev->dev,
				"WLED waiting for the display to turn on + 50ms to fit display power sequence\n");
			if (!(ursa_display_enabled || ursa_waitqueue_shutdown))
				wait_event_timeout(ursa_display_on_waitqueue, ursa_display_enabled || ursa_waitqueue_shutdown, msecs_to_jiffies(1000));
			//TODO: Replace this msleep with a trigger on 3 frames sent, rather than assuming Android will send the frames we need
			msleep(120);
		}
#endif
		val = WLED_BOOST_ON;
	...
	if (level <= WLED_DOWNSCALE_THRESHOLD_INPUT)
	{
		//Input of 1/255 (AKA 16/4095) should give MIN_DUTY_CYCLE_DOWNSCALED
		//Input of 13/255 (AKA 208/4095) should give MAX_DUTY_CYCLE_DOWNSCALED
		//Do a linear interpolation between the two of them

		if (level > 0 && level <= WLED_DOWNSCALE_MIN_INPUT)
		{
			//Clamp at this minimum duty cycle to avoid flicker
			duty = WLED_MIN_DUTY_CYCLE_DOWNSCALED;
		}
		else
		{
			//Linear interpolation, y=ax+b,
			// as solved for y=MAX_DUTY_CYCLE, x=DOWNSCALE_THRESHOLD
			// and solved for y=MIN_DUTY_CYCLE, x=DOWNSCALE_MIN_INPUT
			duty = ( ( WLED_MAX_DUTY_CYCLE_DOWNSCALED - WLED_MIN_DUTY_CYCLE_DOWNSCALED ) * level)
				/  (WLED_DOWNSCALE_THRESHOLD_INPUT-WLED_DOWNSCALE_MIN_INPUT)
				+ WLED_MAX_DUTY_CYCLE_DOWNSCALED
				- (
					( WLED_DOWNSCALE_THRESHOLD_INPUT*( WLED_MAX_DUTY_CYCLE_DOWNSCALED - WLED_MIN_DUTY_CYCLE_DOWNSCALED ))
					/ (WLED_DOWNSCALE_THRESHOLD_INPUT-WLED_DOWNSCALE_MIN_INPUT)
				);
		}
	}
...
static int __devinit qpnp_wled_init(struct qpnp_led_data *led)
{
...
#ifdef CONFIG_MACH_URSA
	rc = qpnp_led_masked_write(led, WLED_MODULATION_SCHEME(led->base),
		0xFF,
		0x0); //analog dimming rather than digital 12 bit resolution, 19.2MHz
	if (rc) {
		dev_err(&led->spmi_dev->dev,
				"WLED_MODULATION_SCHEME reg write failed(%d)\n", rc);
		return rc;
	}
#endif
	...
	for (i = 0; i < num_wled_strings; i++) {
		...
		rc = qpnp_led_masked_write(led,
			WLED_IDAC_DLY_REG(led->base, i), 0xFF,
			0x1);
		if (rc) {
			dev_err(&led->spmi_dev->dev,
				"WLED WLED_IDAC_DLY_REG (SYNC_DELAY) reg write failed(%d)\n", rc);
			return rc;
		}
...
static int __devinit qpnp_flash_init(struct qpnp_led_data *led)
{
	...
	/* URSA-2058: Disable thermal derating */
#ifdef CONFIG_MACH_URSA
	rc = qpnp_led_masked_write(led, (led->base + 0x52), 0xff, 00);
	if (rc) {
		dev_err(&led->spmi_dev->dev,
			"Thermal derating write failed(%d)\n", rc);
		return rc;
	}
#endif
...
static int __devexit qpnp_leds_remove(struct spmi_device *spmi)
{
	...
	ursa_waitqueue_shutdown = 1;
	wake_up(&ursa_display_on_waitqueue);
