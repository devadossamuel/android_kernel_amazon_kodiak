...
static struct spmi_controller *the_ctrl;

int spmi_anon_ext_register_readl(u8 sid, u16 addr, u8 *buf, int len)
{
	if (!the_ctrl) {
		pr_err("Could not perform read; no SPMI controller has registered yet\n");
		return -ENODEV;
	}

	return spmi_ext_register_readl(the_ctrl, sid, addr, buf, len);
}
EXPORT_SYMBOL_GPL(spmi_anon_ext_register_readl);

int spmi_anon_ext_register_writel(u8 sid, u16 addr, u8 *buf, int len)
{
	if (!the_ctrl) {
		pr_err("Could not perform write; no SPMI controller has registered yet\n");
		return -ENODEV;
	}

	return spmi_ext_register_writel(the_ctrl, sid, addr, buf, len);
}
EXPORT_SYMBOL_GPL(spmi_anon_ext_register_writel);
...
static int spmi_register_controller(struct spmi_controller *ctrl)
{
	...
	if (the_ctrl)
		pr_err("Another SPMI controller already registered.\n");
	else
		the_ctrl = ctrl;
