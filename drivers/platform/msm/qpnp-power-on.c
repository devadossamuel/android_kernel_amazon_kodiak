...
#if defined(CONFIG_AMAZON_METRICS_LOG)
#define QPNU_PON_XVDD_RB_SPARE(base)            (base + 0x8E)
#endif
...
#if defined(CONFIG_AMAZON_METRICS_LOG)
int qpnp_pon_read_xvdd_reg(u8* value)
{
	int rc;
	struct qpnp_pon *pon = sys_reset_dev;
	...
	rc = spmi_ext_register_readl(pon->spmi->ctrl, pon->spmi->sid,
					QPNU_PON_XVDD_RB_SPARE(pon->base),
					value, 1);
	if (rc) {
		dev_err(&pon->spmi->dev,
				"Unable to read xvdd PMIC register\n");
		return rc;
	}

	return rc;
}
EXPORT_SYMBOL(qpnp_pon_read_xvdd_reg);

int qpnp_pon_write_xvdd_reg(u8 value)
{
	int rc;
	struct qpnp_pon *pon = sys_reset_dev;
...
EXPORT_SYMBOL(qpnp_pon_write_xvdd_reg);
