...
//This function is unused on ursa, because the display and touch
// power sequence GPIOs are controlled by mdss_dsi_panel_power_on
// directly
void mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	//DO NOT MODIFY THIS FUNCTION WITHOUT CONSULTING THE URSA BSP TEAM
    pr_err("Error!  Entered unsupported function %s on URSA platform\n", __func__);
}
#else
			       ...
			       struct mdss_panel_common_pdata *panel_data)
{
	...
	dsi_debug = of_property_read_bool(pdev->dev.of_node,"qcom,mdss-dsi-debug-enabled");
	
	if (dsi_debug){
		pr_err("%s:%d, dsi debug enabled", __func__, __LINE__);
		panel_data->panel_info.dsi_debug_enabled = 1;
	}
