...
extern wait_queue_head_t ursa_display_on_waitqueue;
extern int ursa_display_enabled;
#endif
...
static int mdss_dsi_regulator_init(struct platform_device *pdev)
{
	...
	{
		pr_err("%s (CONFIG_MACH_URSA) Error! attempting to re-initialize \n"
			"display regulators.  Please verify that only one display panel config is loaded\n",
			__func__
		);
	...
	if (ursa_board_revision() == URSA_REVISION_P0)
	{
	...
	if (ursa_board_revision() == URSA_REVISION_P0)
	{
		ret = regulator_set_voltage(dsi_drv->vdd_io_panel_vreg, 1800000,
				1800000);
		if (ret) {
...
// without consulting the Ursa BSP team

//tA, tB, etc are timings specified by our hardware vendor
// see "P1_Display_Verification_Document_Rev8.0" for more
// information

static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata, int enable)
{
	//DO NOT MODIFY THIS FUNCTION WITHOUT CONSULTING THE URSA BSP TEAM
	...
	if (enable) {
		...
		{
			ret = regulator_set_optimum_mode(
				(ctrl_pdata->shared_pdata).vdd_io_panel_vreg, 100000);
			if (ret < 0) {
				pr_err("%s: vdd_io_panel_vreg set opt mode failed.\n",
					__func__);
				return ret;
			}
		}
		...
		//The display + touch power are now online

		// Used to ensure that the backlight doesn't turn on before the
		// display has been on for 50+ ms, to avoid seeing a flash of
		// garbage on the screen

		//The waitqueue gets declared as part of the kernel's init
		// stage, well before this code can be loaded, and neither the
		// LED driver where the queue is declared nor the display
		// driver can be run as a module or unloaded

		ursa_display_enabled = 1;
		wmb();
		...
		{
			ret = regulator_set_optimum_mode(
				(ctrl_pdata->shared_pdata).vdd_io_panel_vreg, 100);
			if (ret < 0) {
				pr_err("%s: vdd_io_vreg set opt mode failed.\n",
					__func__);
				return ret;
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_CONT_SPLASH_FINISH:
		...
		break;
	case MDSS_EVENT_DSI_DUMP_REGS:
		if(pdata->panel_info.dsi_debug_enabled){
			dumpreg();
		}else {
			pr_err("%s: Debugging not enabled \n",
				 __func__);
		}
		break;
	case MDSS_EVENT_DSI_PLL_DETECT_UNLOCK:
		/*Try turning OFF/ON the clock to lock again*/
		pr_err("%s MDSS_EVENT_DSI_PLL_DETECT_UNLOCK \n",__func__);
		mdss_dsi_clk_status(ctrl_pdata);
		break;
...
#if defined(CONFIG_MACH_URSA)
	//NOTE: This function does not clean up after itself on failure,
	// so don't attempt to remove/insert the display driver without
	// rebooting

	if (ursa_board_revision() != URSA_REVISION_P0)
	{
		ctrl_pdata->touch_rst_gpio = of_get_named_gpio(pdev->dev.of_node,
							"qcom,touch-rst-gpio", 0);
