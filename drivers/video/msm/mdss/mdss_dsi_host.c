...
void dumpreg(void)
{
	...
	rc = spmi_anon_ext_register_readl(1, 0x4B46, &reg, 1);
	if (rc)
		pr_err("spmi read failed\n");
	else
		pr_alert("LDO 12 EN_CTL=0x%02X: %s\n", reg, reg & 0x80 ? "enabled" : "disabled");

	//static unsigned char *dsi_ctrl_base_pll;
	pr_err("%s: =============DSI Controller Reg DUMP==============\n", __func__);
	...
	pr_err("%s: =============DSI PLL Reg DUMP==============\n", __func__);

	for(i=0; i< 40; i++){
		tmp0x0 = MIPI_INP(dsi_ctrl_base+0x200+(i*16)+0x0);
		tmp0x4 = MIPI_INP(dsi_ctrl_base+0x200+(i*16)+0x4);
		tmp0x8 = MIPI_INP(dsi_ctrl_base+0x200+(i*16)+0x8);
		tmp0xc = MIPI_INP(dsi_ctrl_base+0x200+(i*16)+0xc);
