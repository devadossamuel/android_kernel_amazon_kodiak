...
static irqreturn_t mdss_irq_handler(int irq, void *ptr)
{
	...
	u32 intr = 0, i;
        static u32 intr_index = 0;
        static u32 intr_cache[IRQ_MAX_CACHE];

	if (!mdata)
		return IRQ_NONE;

	intr = MDSS_MDP_REG_READ(MDSS_REG_HW_INTR_STATUS);
	intr_cache[intr_index % IRQ_MAX_CACHE] = intr;
	intr_index++;
	mdata->irq_buzy = true;

	if (intr & MDSS_INTR_MDP) {
		if (!mdata->clk_ena) {
			for(i = 0; i < IRQ_MAX_CACHE; i++)
				pr_err("Intr[%d] = %x \n",i, intr_cache[i]);
			panic("mdss_irq_handler: Accessing MDSS clocks while disabled\n");			
		}
