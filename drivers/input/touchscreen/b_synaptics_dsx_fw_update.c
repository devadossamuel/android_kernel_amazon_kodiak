...
#define FW_IMAGE_NAME_INX_EVT "synaptics/PR1518120-s2718br_00000014.img"
#define FW_IMAGE_NAME_INX_ITO "synaptics/PR1518120-s2718br_00000016.img"
#define FW_IMAGE_NAME_INX_IZO "synaptics/PR1518120-s2718br_20131210_6AA.img"
#define FW_IMAGE_NAME_INX_IXO "Insert_IXO_Firmware_Here"
//This firmware is only valid on DDIC "cut 2" devices from JDI
#define FW_IMAGE_NAME_JDI "synaptics/PR1646488-s3310bt_000052EA.img"
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source) {
		...
		if (ursa_board_revision() >= URSA_REVISION_DVT)
		{
			switch ( ursa_ddic_id_code&0xE0 )
			{
			case 0x60:
				dev_err(&fwu->rmi4_data->i2c_client->dev,
						"%s: Innolux IXO panel detected, no firmware available (0x%x)\n",
						__func__, ursa_ddic_id_code);
				retval = -EINVAL;
				goto exit;
				//strncpy(fwu->image_name, FW_IMAGE_NAME_INX_IXO, MAX_IMAGE_NAME_LEN);
				//break;
			case 0xE0:
				strncpy(fwu->image_name, FW_IMAGE_NAME_INX_IZO, MAX_IMAGE_NAME_LEN);
				break;
			case 0x40:
				strncpy(fwu->image_name, FW_IMAGE_NAME_INX_ITO, MAX_IMAGE_NAME_LEN);
				break;
			case 0xC0:
				strncpy(fwu->image_name, FW_IMAGE_NAME_INX_EVT, MAX_IMAGE_NAME_LEN);
				break;
			case 0x80:
