...
#define BQ27541_LIMIT_COUNT		3	// generic/default limit count
...
enum {
	...
	BQ27541_STATS_FYI_e,
	BQ27541_STATS_RTC_CNT_e,
	BQ27541_STATS_RES_CNT_e,
	BQ27541_STATS_e_MAX,
};
 ...
 * @rtc_alarm:			RTC alarm structure
 * @wake_lock:			a general wake lock (for resume, ...)
 ...
 * @status.info.battery_man_buffer: battery manufacturer buffer
 ...
 * @charge_monitor.suspend_time: accumulates suspend time
...
struct bq27541_chip {
	...
	struct {
		...
		struct {
			...
			int		temp_hi_shutdown_count;
			int		volt_lo_shutdown_mv;
			int		volt_lo_shutdown_count;
		} limits;
		struct {
			unsigned long	next;
			int		seconds;
 ...
 */
static unsigned long bq27541_get_wall_seconds(void)
{
	struct timeval ts = ktime_to_timeval(ktime_get_real());
	return (unsigned long)ts.tv_sec + (ts.tv_usec > 500000);
}
 ...
 * YYYY-MM-DD-hh-mm-ss
 */
static int bq27541_get_wall_string(char * buffer, unsigned long seconds)
{
	struct tm time;
	const int len = 19;

	if (seconds == 0) seconds = bq27541_get_wall_seconds();
	time_to_tm((time_t)seconds, 0, &time);
	snprintf(buffer, len+1, "%04d-%02d-%02d-%02d-%02d-%02d",
			(int)time.tm_year + 1900, time.tm_mon + 1, time.tm_mday,
 ...
 * RTC alarm callback.
 */
static void bq27541_rtc_alarm_callback(struct alarm * alarm)
{
	struct bq27541_chip *chip = container_of(
			alarm, struct bq27541_chip, rtc_alarm);
	chip->stats.count[BQ27541_STATS_RTC_CNT_e]++;
}

/**
 * Set/clear RTC alarm. Used during suspend.
 */
static void bq27541_rtc_alarm_set(struct bq27541_chip * chip, int seconds)
{
	if (seconds) {
		ktime_t interval = ktime_set(seconds, 0);
		ktime_t slack = ktime_set(seconds / 2, 0); // +50%
		ktime_t now = ktime_get_real();
		ktime_t next = ktime_add(now, interval);
		alarm_start_range(&chip->rtc_alarm, next,
				ktime_add(next, slack));
	} else {
		alarm_cancel(&chip->rtc_alarm);
	}
}
 ...
 */
static int bq27541_vadc_read(enum qpnp_vadc_channels channel, int * value)
{
	struct qpnp_vadc_result result;
	int rc = qpnp_vadc_read(channel, &result);
	if (rc) {
 ...
 */
static int bq27541_iadc_read(enum qpnp_iadc_channels channel, int * value)
{
	struct qpnp_iadc_result result;
	int rc = qpnp_iadc_read(channel, &result);
	if (rc) {
		pr_err("error %d, channel %d\n", rc, channel);
		return rc;
	}
...
/**
 * Log a metric to track battery state.
 */
static void bq27541_log_battery_metric(struct bq27541_chip * chip, int type)
{
...
static int bq27541_read_current_now_ua(struct bq27541_chip * chip)
{
	...
	bq27541_iadc_read(EXTERNAL_RSENSE, &val);
	return -val;
}
...
static int bq27541_read_voltage_now_uv(struct bq27541_chip * chip)
{
	...
	if (chip->status.battery_present &&
		...
		return val * 1000;
	bq27541_vadc_read(VBAT_SNS, &val);
	return val;
}
		...
		int seal, unsigned int key)
{
	...
	pr_info("%d (%d), %d, k0 = %04x, k1 = %04x, %d (%d)\n",
...
static int bq27541_procfs_status(char * pPage, char ** ppStart, off_t off,
		int count, int * pEof, void * refData)
{
	struct bq27541_chip * chip = refData;
	if (off) return 0;
	return snprintf(pPage, count,
			"mVolts: %d\n"
			"mAmps: %d\n"
			"temp: %d\n"
			"rsoc: %d\n"
...
static int bq27541_procfs_config(char * pPage, char ** ppStart, off_t off,
		int count, int * pEof, void * refData)
{
	struct bq27541_chip * chip = refData;
	if (off) return 0;
...
static int bq27541_procfs_dfd_read(char * pPage, char ** ppStart, off_t off,
		int count, int * pEof, void * refData)
{
	struct bq27541_chip * chip = refData;
	int i, l, rc;
	...
	bq27541_release_busy(chip);
	if (!rc) {
		rc = snprintf(pPage, count, "%d %d %d =", chip->procfs.class,
				chip->procfs.offset, chip->procfs.length);
		count -= rc;
		for (i=0;i < chip->procfs.length;i++) {
			l = snprintf(pPage + rc, count, " %02x",
					chip->procfs.buffer[i]);
			rc += l;
			count -= l;
		}
		rc += snprintf(pPage + rc, count, "\n");
	}
	return rc;
}
 ...
 */
static int bq27541_procfs_dfd_write(struct file * file, const char * data,
		unsigned long count, void * refData)
{
	struct bq27541_chip * chip = refData;
	char *p, prefix[64];
	unsigned long v;
	...
	if (count < 1 || count >= sizeof(chip->procfs.local))
		return -EINVAL;
	if (copy_from_user(chip->procfs.local, data, count))
		return -EFAULT;
...
static int bq27541_procfs_sealed_read(char * pPage, char ** ppStart, off_t off,
		int count, int * pEof, void * refData)
{
	struct bq27541_chip * chip = refData;
	...
	bq27541_release_busy(chip);
	return snprintf(pPage, count, "sealed: %s, full access: %s\n",
 ...
 */
static int bq27541_procfs_sealed_write(struct file * file, const char * data,
		unsigned long count, void * refData)
{
	struct bq27541_chip * chip = refData;
...
static int bq27541_procfs_data_read(char * pPage, char ** ppStart, off_t off,
		int count, int * pEof, void * refData)
{
	struct bq27541_chip * chip = refData;

	if (off) return 0;
	return snprintf(pPage, count, "%02x %04x = %04x %u %d\n",
 ...
 */
static int bq27541_procfs_data_write(struct file * file, const char * data,
		unsigned long count, void * refData)
{
	struct bq27541_chip * chip = refData;
	unsigned long value;
...
static int bq27541_procfs_help_read(char * pPage, char ** ppStart, off_t off,
		int count, int * pEof, void * refData)
{
	...
	int remaining = sizeof(help) - off - 1;
	if (remaining <= 0) return 0;
	if (remaining > count) remaining = count;
	memcpy(pPage, help + off, remaining);
	return remaining;
}

#if defined(CONFIG_BQ27541_STATISTICS)
static int bq27541_procfs_stats_read(char * pPage, char ** ppStart,
		off_t off, int count, int * pEof, void * refData)
{
	static const char * stat_names[] = {
		...
		[BQ27541_STATS_RTC_CNT_e] = "RTC_CNT",
		[BQ27541_STATS_RES_CNT_e] = "RES_CNT",
	};
	struct bq27541_chip * chip = refData;
	int size = 0;
	if (off == 0) {
		int e = bq27541_get_wall_seconds() - chip->stats.start_time;
		int i, h, m, s;
		h = e / 3600;
		s = e - h * 3600;
		m = s / 60;
		s -= m * 60;
		size += snprintf(pPage + size, count - size,
				"ELAPSED_TIME = %d:%02d:%02d\n", h, m, s);
...
static void bq27541_procfs_stats_clear(struct bq27541_chip * chip)
{
	...
	chip->stats.start_time = bq27541_get_wall_seconds();
}

static int bq27541_procfs_stats_write(struct file * file,
		const char * data, unsigned long count, void * refData)
{
	struct bq27541_chip * chip = refData;
	static const char clear_token[] = { 'c', 'l', 'e', 'a', 'r' };
	char buf[sizeof(clear_token)];
	if (count >= sizeof(clear_token) &&
			copy_from_user(buf, data, sizeof(clear_token)) == 0 &&
...
static void bq27541_procfs_init(struct bq27541_chip * chip)
{
	...
	if (dir) {
		...
		dent = create_proc_entry("status", S_IFREG | 0400, dir);
		if (dent) {
			dent->data = chip;
			dent->read_proc = bq27541_procfs_status;
		}
		dent = create_proc_entry("config", S_IFREG | 0400, dir);
		if (dent) {
			dent->data = chip;
			dent->read_proc = bq27541_procfs_config;
		}
		dent = create_proc_entry("dfd", S_IFREG | 0600, dir);
		if (dent) {
			dent->data = chip;
			dent->read_proc = bq27541_procfs_dfd_read;
			dent->write_proc = bq27541_procfs_dfd_write;
		}
		dent = create_proc_entry("sealed", S_IFREG | 0600, dir);
		if (dent) {
			dent->data = chip;
			dent->read_proc = bq27541_procfs_sealed_read;
			dent->write_proc = bq27541_procfs_sealed_write;
		}
		dent = create_proc_entry("data", S_IFREG | 0600, dir);
		if (dent) {
			dent->data = chip;
			dent->read_proc = bq27541_procfs_data_read;
			dent->write_proc = bq27541_procfs_data_write;
		}
		dent = create_proc_entry("help", S_IFREG | 0400, dir);
		if (dent) {
			dent->data = chip;
			dent->read_proc = bq27541_procfs_help_read;
		}
#if defined(CONFIG_BQ27541_STATISTICS)
		dent = create_proc_entry("statistics", S_IFREG | 0600, dir);
		if (dent) {
			dent->data = chip;
			dent->read_proc = bq27541_procfs_stats_read;
			dent->write_proc = bq27541_procfs_stats_write;
		}
#endif
#if defined(CONFIG_BQ27541_FLASHSTREAM)
		if (chip->status.battery_present) {
...
static void bq27541_pmic_smbb_snapshot(struct bq27541_chip * chip)
{
	...
	pr_info("X %4dmv %4dmv %4dmv %4dma %4dma %4dma"
...
static void bq27541_snapshot(struct bq27541_chip * chip)
{
	u8 pmic_pwr, usb_sts;
	char tbuf[32];
	bq27541_get_wall_string(tbuf, 0);
...
static int bq27541_battery_id(struct bq27541_chip * chip)
{
	...
	rc = bq27541_vadc_read(LR_MUX2_BAT_ID, &id);
	if (rc) {
	...
	pr_info("mode = %d, board = %d, batt_id = %d, batt_man = \"%s\"\n",
...
static void bq27541_read_mib(struct bq27541_chip * chip)
{
	...
	// decode manufacturer
	if (chip->status.mib.pack_serial_number[0] != 'B')
		return;
	...
	if (ursa_board_revision() < URSA_REVISION_EVT) {
		 ...
		 * BDS		1	Desay		ATL
		 * BDS		2	Desay		LGC
		 * BSW		1	Sunwoda		Samsung
		 * BSW		2	Sunwoda		Coslight
		 *
		 * DVT
		 *
		 * BDS		3	Desay		ATL
		 * BDS		4	Desay		LGC
		 * BSW		4	Sunwoda		Coslight
		 *
		 * PVT
		 *
		 * BDS		5	Desay		LGC
		 * BSW		5	Sunwoda		Coslight
		 *
		 * ---
		 *
		 * BMC			McNair
		...
		char *psn = chip->status.mib.pack_serial_number;
		char *pv = 0, *cv = 0;
		if (memcmp("BDS", psn, 3) == 0) {
			pv = "Desay";
			if (rev == '1' || rev == '3')
				cv = "ATL";
			else if (rev == '2' || rev == '4' || rev == '5')
				cv = "LGC";
		} else if (memcmp("BMC", psn, 3) == 0) {
			pv = "McNair";
		} else if (memcmp("BSW", psn, 3) == 0) {
			pv = "Sunwoda";
			if (rev == '1')
				cv = "Samsung";
			else if (rev == '2' || rev == '4' || rev == '5')
		...
		if (pv) {
			snprintf(chip->status.info.battery_man_buffer,
				sizeof(chip->status.info.battery_man_buffer),
					"%s/%s", pv, cv ? cv : "");
			chip->status.info.battery_man =
					chip->status.info.battery_man_buffer;
		}
	}
...
static void bq27541_update_status(struct bq27541_chip * chip)
{
	...
	vbat = 0;
	bq27541_vadc_read(VBAT_SNS, &vbat);
	vbat /= 1000;
	...
	if (!chip->status.battery_present) {
		...
		temp = 250;
		ai = 0;
		bq27541_vadc_read(LR_MUX1_BATT_THERM, &temp);
		bq27541_iadc_read(EXTERNAL_RSENSE, &ai);
		ai = -ai / 1000;
	 ...
	 */
	/**
	 * Addendum: SOC from GG is no longer held at 1%, while Vbat is greater
	 * than Vmin. Doing so allowed low loads to fully deplete the cell, but
	 * a high load to trip the lower guard limit, bypassing graceful shut
	 * down.
	 * if (soc == 0) soc = 1;
	 * if (vsoc == 0) vsoc = 1;
	 */
	...
	} else if (chip->status.zero_soc_count++ >= BQ27541_LIMIT_COUNT) {
		soc = vsoc = 0;
	}
		...
		int reschedule)
{
	...
	if (chip->status.snapshot.next && now >= chip->status.snapshot.next) {
		...
		bq27541_log_battery_metric(chip, 1);
...
static int bq27541_charge_monitor(struct bq27541_chip * chip, int force)
{
	...
	if ((charging != chip->charge_monitor.charging) || force ||
			(!charging && soc == 0)) {
		unsigned long now = bq27541_get_wall_seconds();
...
//				",stop_reason=%d;CT;1" full,usb,health
					...
					chip->charge_monitor.charging
							? "" : "dis",
					duration,
					chip->charge_monitor.suspend_time,
		...
		chip->charge_monitor.suspend_time = 0;
	}
...
static int bq27541_screen_monitor(struct bq27541_chip * chip, int force)
{
	...
	if (chip->screen_monitor.delay == 0) {
		chip->screen_monitor.temp_soc = bq27541_get_status(chip,
				BATT_STATUS_CAPACITY);
		chip->screen_monitor.temp_time = bq27541_get_wall_seconds();
	}
...
#define MIN_SECONDS_DISCHARGE_METRIC (60 * 30)
#define MIN_DELTA_SOC_METRIC 500

static int bq27541_usb_monitor(struct bq27541_chip * chip, int force)
{
	union power_supply_propval ret;
	int usb_present;

	if (!chip->usb_monitor.usb_psy)
		chip->usb_monitor.usb_psy = power_supply_get_by_name("usb");
	if (!chip->usb_monitor.usb_psy)
		return 0;
	chip->usb_monitor.usb_psy->get_property( chip->usb_monitor.usb_psy,
			POWER_SUPPLY_PROP_ONLINE, &ret);
	usb_present = !!ret.intval;
	if ((usb_present != chip->usb_monitor.prev_usb_present) ||
			(force && !chip->usb_monitor.prev_usb_present)) {
		__kernel_time_t seconds;
		struct timespec ts;
		long soc, duration, delta_soc;

		getnstimeofday(&ts);
		seconds = ts.tv_sec + (ts.tv_nsec > 500000000);
		duration = seconds - chip->usb_monitor.prev_seconds;
		soc = (long)bq27541_get_status(chip, BATT_STATUS_RM) * 10000;
		soc /= (long)bq27541_get_status(chip, BATT_STATUS_FCC);
		delta_soc = chip->usb_monitor.prev_soc - soc;

		if ((usb_present || force) && delta_soc > 0 &&
				duration < (7 * 24 * 3600) &&
				(duration >= MIN_SECONDS_DISCHARGE_METRIC ||
					delta_soc >= MIN_DELTA_SOC_METRIC)) {
			char buf[128];
			long rate = (delta_soc * 3600) / duration;
			snprintf(buf, sizeof(buf),
					"discharge_rate:usb_detached:"
					"duration=%ld;TI;1,"
					"begin_soc=%ld;CT;1,"
					"end_soc=%ld;CT;1,"
					"rate=%ld.%02ld;CT;1:NR",
					duration, chip->usb_monitor.prev_soc,
					soc, rate / 100, rate % 100);
			bq27541_log_metric("power", buf);
		}
		chip->usb_monitor.prev_seconds = seconds;
		chip->usb_monitor.prev_soc = soc;
	}
	chip->usb_monitor.prev_usb_present = usb_present;
	return 0;
}
...
static int bq27541_check_limits(struct bq27541_chip * chip, int status_delta)
{
	...
	if (chip->status.limits.temp_hi_shutdown_dk) {
		...
		if (temp <= DK_TO_DC(chip->status.limits.temp_hi_shutdown_dk)) {
			chip->status.limits.temp_hi_shutdown_count = 0;
		} else if (chip->status.limits.temp_hi_shutdown_count++
				>= BQ27541_LIMIT_COUNT) {
			char buf[128];
			bq27541_snapshot(chip);
			snprintf(buf, sizeof(buf),
	...
	if (chip->status.limits.volt_lo_shutdown_mv) {
		...
		if (volt >= chip->status.limits.volt_lo_shutdown_mv) {
			chip->status.limits.volt_lo_shutdown_count = 0;
		} else if (chip->status.limits.volt_lo_shutdown_count++
				>= BQ27541_LIMIT_COUNT) {
			char buf[128];
			bq27541_snapshot(chip);
			snprintf(buf, sizeof(buf),
...
static void bq27541_status_work_do_work(struct bq27541_chip * chip)
{
	...
	notify_mask |= bq27541_usb_monitor(chip, 0);
...
/**
 * Runs periodically, from the system default work queue.
 */
				 ...
				 const struct i2c_device_id * id)
{
	...
	if (rc) {
		pr_info("vadc not ready: %d\n", rc);
		return -EPROBE_DEFER;
	}
	rc = qpnp_iadc_is_ready();
	if (rc) {
		pr_info("iadc not ready: %d\n", rc);
		return -EPROBE_DEFER;
	}

	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		pr_info("check i2c functionality failed\n");
		return -ENODEV;
	}
	...
	wake_lock_init(&chip->wake_lock, WAKE_LOCK_SUSPEND,
			BQ27541_GG_DEV_NAME);
	alarm_init(&chip->rtc_alarm, ANDROID_ALARM_RTC_WAKEUP,
			bq27541_rtc_alarm_callback);
...
static int bq27541_pm_suspend_prepare(struct device *dev)
{
	struct bq27541_chip *chip = dev_get_drvdata(dev);
	if (!chip) return 0;
	chip->suspended = 1;
	...
	bq27541_status_work_reschedule(chip);
#endif
	bq27541_log_battery_metric(chip, 2);
	chip->suspend_soc = bq27541_get_status(chip, BATT_STATUS_CAPACITY);
	chip->suspend_time = bq27541_get_wall_seconds();
	if (chip->status.sleep_real_seconds)
		bq27541_rtc_alarm_set(chip, chip->status.sleep_real_seconds);
	return 0;
}

static void bq27541_pm_resume_complete(struct device *dev)
{
	struct bq27541_chip *chip = dev_get_drvdata(dev);
	if (!chip) return;
	wake_lock(&chip->wake_lock);
	chip->suspended = 0;
	chip->stats.count[BQ27541_STATS_RES_CNT_e]++;
	bq27541_rtc_alarm_set(chip, 0);
	bq27541_status_work_reschedule(chip);
	if (chip->suspend_time) {
		unsigned long elapsed_ms = (bq27541_get_wall_seconds() -
		...
		snprintf(buf, sizeof(buf),
				...
				delta_soc, elapsed_ms);
		bq27541_log_metric("drain_metrics", buf);
		chip->charge_monitor.suspend_time += elapsed_ms;
		chip->suspend_time = 0;
		bq27541_log_battery_metric(chip, 3);
	}
	wake_unlock(&chip->wake_lock);
}

static const struct dev_pm_ops bq27541_pm_ops = {
	.prepare = bq27541_pm_suspend_prepare,
	.complete = bq27541_pm_resume_complete,
};
