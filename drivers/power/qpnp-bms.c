...
static int set_battery_data(struct qpnp_bms_chip *chip)
{
	...
	// Use an approximate battery profile.
	struct bms_battery_data *batt_data = &projb_v2_2410_data;
	// Read and report battery id, but don't do anything with it.
...
static int __devinit qpnp_bms_probe(struct spmi_device *spmi)
{
	...
	chip->bms_psy.name = "qpnp-bms";
	chip->bms_psy.type = POWER_SUPPLY_TYPE_UNKNOWN;
	...
	chip->bms_psy.num_supplicants = 0;
#else
