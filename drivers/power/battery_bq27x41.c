...
#define BQ27X41_DEVICE_TYPE_V2		0x0741	/* Device type of the gas gauge */
...
static void battery_handle_work(struct work_struct *work)
{
	...
	if (info->battery_voltage <= BQ27X41_VOLT_CRIT_THRESHOLD) {
		...
		snprintf(buf, sizeof(buf),
			"bq27x41:def:critical_shutdown=1:");
...
#if defined(CONFIG_AMAZON_METRICS_LOG) && defined(CONFIG_EARLYSUSPEND)
static void bq27x41_early_suspend(struct early_suspend *handler)
{
...
static void bq27x41_late_resume(struct early_suspend *handler)
{
	struct bq27x41_info *info = container_of(handler,
			struct bq27x41_info, early_suspend);
...
static int bq27x41_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (dev_ver != BQ27X41_DEVICE_TYPE && dev_ver != BQ27X41_DEVICE_TYPE_V2) {
	...
	info->early_suspend.suspend = bq27x41_early_suspend;
	info->early_suspend.resume = bq27x41_late_resume;
