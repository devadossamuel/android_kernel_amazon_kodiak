 ...
 */
#ifdef CONFIG_MACH_URSA
#define WCD9XXX_CS_MEAS_INVALD_RANGE_LOW_MV 125
#else
...
static int cur_insert_status = 0;
...
#ifdef CONFIG_MBHC_IMPEDANCE_DETECTION
 ...
 */
static const char * wcd9xxx_mbhc_plug_type_strings[] = {
	"INVALID", "NOTHING", "HEADSET", "HEADPHONES",
	"LINEOUT", "HEADSET_OMTP"
};

#define METRICS_INSERT_REMOVAL_TAG "wcd9xxx_report_plug"
#endif
				...
				struct snd_soc_jack *jack, int status, int mask)
{
	...
	cur_insert_status = mask ? status : 0;
}
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		snprintf(metrics_buf, sizeof(metrics_buf),
			"%s:jack:unplugged=1;CT;1:NR", METRICS_INSERT_REMOVAL_TAG);
		log_to_metrics(ANDROID_LOG_INFO, "AudioJackEvent", metrics_buf);
#endif
...
#ifdef CONFIG_AMAZON_METRICS_LOG
		/* Note: plug_type starts at -1 */
		snprintf(metrics_buf, sizeof(metrics_buf),
			"%s:jack:plugged=1;CT;1,state_%s=1;CT;1:NR",
			METRICS_INSERT_REMOVAL_TAG,
			wcd9xxx_mbhc_plug_type_strings[mbhc->current_plug + 1]);
		log_to_metrics(ANDROID_LOG_INFO, "AudioJackEvent", metrics_buf);
#endif
			  ...
			  unsigned long event_state)
{
	...
	sz = size - 1;
	pr_debug("%s: d->mic_bias = %d, sz = %d", __func__, d->mic_bias, sz);
	for (i = 0, d = dt, ch = 0; i < sz; i++, d++) {
		...
		pr_debug("%s: %d) dce_z = %d, mb_mv = %d, hs_max = %d, no_mic = %d, highhph_cnt = %d\n",
			__func__, i, dce_z, mb_mv, hs_max, no_mic, highhph_cnt);
	...
	for (i = 0, d = dt; i < sz; i++, d++) {
		...
		if (!d->swap_gnd && !d->mic_bias &&
			...
			pr_debug("%s: Invalid, delta %dmv, %dmv and %dmv %dmv\n",
				 __func__, d->_vdces, minv, maxv, delta_thr);
			type = PLUG_TYPE_INVALID;
			goto exit;
		} else if (d->swap_gnd) {
			dgnd = d;
...
#ifndef CONFIG_MBHC_IMPEDANCE_DETECTION
			      ...
			      size_t count, loff_t *pos)
{
	...
	strcat(buffer, buffer2);
	n += scnprintf(buffer2, size, "cur_insert_status = %d\n",
		cur_insert_status);
...
#endif /* CONFIG_MBHC_IMPEDANCE_DETECTION */
